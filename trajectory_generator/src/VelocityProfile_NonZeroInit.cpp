...
VelocityProfile_NonZeroInit::VelocityProfile_NonZeroInit(double _maxvel, double _maxacc) :
		...
		trajSign(1.0), syncSign(1.0), maxVel(abs(_maxvel)), maxAcc(abs(_maxacc)), initPos(0.0), finalPos(0.0), initVel(0.0), finalVel(
				0.0), initTime(0.0)
...
double VelocityProfile_NonZeroInit::SubProfileBuilder(double fPos, double iPos, double iVel, double fVel, double iTime)
{
	...
	else
	{
		...
		if (peakVel < maxVel)
		{
			...
			sp2.push_back(
					subVelProfiles[0][1] + (T20 - iTime) * (subVelProfiles[0][2] + 0.5 * (T20 - iTime) * subVelProfiles[0][3]));
		...
		else
		{
			...
			double T30 = 1 / (maxVel)
					* (trajSign * Dp + (fVel * fVel + iVel * iVel - trajSign * 2 * maxVel * iVel) / (2 * maxAcc));
...
///////////////////
		else
		{
			...
			{
				constVel = (trajSign * (finalPos - initPos) - 0.5 * initVel * initVel / maxAcc)
						/ (synchroTime - initVel / (trajSign * maxAcc));
